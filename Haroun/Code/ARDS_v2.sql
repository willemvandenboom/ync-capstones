...
where datetime_diff(mv.starttime ,bgfd.charttime,HOUR) <= 24
), 

----------------------------------------------------------------------------------------------------------------

cohort as -- patient has ARDS if at least one PFratio calculated on the first day is < 200
(
...
and duration_hours <= 48 -- only include patients who were on mechvent for < 48 hours 
-- this adds up 5806 recoreds corresponding to unique hadm_id

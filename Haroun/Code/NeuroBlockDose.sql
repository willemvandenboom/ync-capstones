...
(
  select
      icustay_id, orderid
    , rate as drug_rate --vaso_rate
    , amount as drug_amount -- vaso_amount
    , starttime
    , endtime
...
select
    ...
    , case
        ...
        when TIME_DIFF(TIME(CHARTTIME), TIME((LAG(CHARTTIME, 1) OVER (partition by icustay_id, drug order by charttime))), HOUR) > 8 then 1
      else null
      end as drug_start
